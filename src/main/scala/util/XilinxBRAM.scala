...
class XilinxSimpleDualPortNoChangeBRAM(width: Int,
    ...
    require(acceptedRamStyles exists (_ == ramStyle))
...
class SDPNoChangeWithStToLoad[T <: Data](addrWidth: Int, dataType: T, numStages: Int=2, initFile: String="")
  extends Module {
    ...
    def read(rdAddr: UInt, rdEn: Bool): T = {
      io.rdAddr := rdAddr
      io.rdEn := rdEn
      io.rdData
    }
...
class XilinxDoublePumped2W2RSDPBRAM(width: Int,
                                       depth: Int,
                                       performance: String="HIGH_PERFORMANCE",
                                       initFile: String="")
                                       extends BlackBox(Map("RAM_WIDTH" -> width,
                                                            ...
                                                            "INIT_FILE" -> initFile))
                                       with HasBlackBoxResource {
    val io = IO(new XilinxDoublePumped2W2RSDPBRAMBlackBoxIO(log2Ceil(depth), width))

    setResource("/XilinxDoublePumped2W2RSDPBRAM.v")
}

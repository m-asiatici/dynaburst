...
#define NUM_BENCHMARKS 18
char* benchmarks[NUM_BENCHMARKS] = {"bcspwr01", "pds-80", "dblp-201", "enron", "webbase-", "eu-2005", "cnr-2000", "in-2004", "youtube", "ljournal", "amazon-2", "flickr",
		"rail4284", "road_usa", "mawi1234", "cont11_l", "cit-Pate", "wikipedi"};
...
#ifdef XPAR_FULLYPIPELINEDSPMVRTL_4_BASEADDR
		,XPAR_FULLYPIPELINEDSPMVRTL_4_BASEADDR
#endif
#ifdef XPAR_FULLYPIPELINEDSPMVRTL_5_BASEADDR
		,XPAR_FULLYPIPELINEDSPMVRTL_5_BASEADDR
#endif
#ifdef XPAR_FULLYPIPELINEDSPMVRTL_6_BASEADDR
		,XPAR_FULLYPIPELINEDSPMVRTL_6_BASEADDR
#endif
#ifdef XPAR_FULLYPIPELINEDSPMVRTL_7_BASEADDR
		,XPAR_FULLYPIPELINEDSPMVRTL_7_BASEADDR
#endif
};
...
#ifdef XPAR_FULLYPIPELINEDSPMVRTL_4_BASEADDR
		,XPAR_AXI_DMA_12_DEVICE_ID
#endif
#ifdef XPAR_FULLYPIPELINEDSPMVRTL_5_BASEADDR
		,XPAR_AXI_DMA_15_DEVICE_ID
#endif
#ifdef XPAR_FULLYPIPELINEDSPMVRTL_6_BASEADDR
		,XPAR_AXI_DMA_18_DEVICE_ID
#endif
#ifdef XPAR_FULLYPIPELINEDSPMVRTL_7_BASEADDR
		,XPAR_AXI_DMA_21_DEVICE_ID
#endif
};
...
#ifdef XPAR_FULLYPIPELINEDSPMVRTL_4_BASEADDR
		,XPAR_AXI_DMA_13_DEVICE_ID
#endif
#ifdef XPAR_FULLYPIPELINEDSPMVRTL_5_BASEADDR
		,XPAR_AXI_DMA_16_DEVICE_ID
#endif
#ifdef XPAR_FULLYPIPELINEDSPMVRTL_6_BASEADDR
		,XPAR_AXI_DMA_19_DEVICE_ID
#endif
#ifdef XPAR_FULLYPIPELINEDSPMVRTL_7_BASEADDR
		,XPAR_AXI_DMA_22_DEVICE_ID
#endif
};
...
#ifdef XPAR_FULLYPIPELINEDSPMVRTL_4_BASEADDR
		,XPAR_AXI_DMA_14_DEVICE_ID
#endif
#ifdef XPAR_FULLYPIPELINEDSPMVRTL_5_BASEADDR
		,XPAR_AXI_DMA_17_DEVICE_ID
#endif
#ifdef XPAR_FULLYPIPELINEDSPMVRTL_6_BASEADDR
		,XPAR_AXI_DMA_20_DEVICE_ID
#endif
#ifdef XPAR_FULLYPIPELINEDSPMVRTL_7_BASEADDR
		,XPAR_AXI_DMA_23_DEVICE_ID
#endif
};
...
int Test_spmv_mult_axis(int num_spmv){
	...
	for(i = 0; i < num_spmv; i++) {
		...
		Xil_DCacheFlushRange((u32)vect_mem, cols * sizeof(float));
...
//	return 0;
  ...
  int num_spmv, cache_divider, benchmark_idx, alwaysFullBurst;
#if FPGAMSHR_EXISTS
  xil_printf("benchmark numAcc robDepth mshrHashTables mshrPerHashTable seBufPerRow seBufRows cacheWays cacheSize alwaysFullBurst totalCycles ");
#else
  ...
  for(benchmark_idx = 0; benchmark_idx < NUM_BENCHMARKS; benchmark_idx++) { // for each benchmark
	   for(num_spmv = 1; num_spmv <= NUM_SPMV; num_spmv++) { // for all numbers of accelerators (num_spmv always 4 in the paper)
      ...
      // effective_cache_size = total_cache_size / (2 ^ cache_divider)
			for(alwaysFullBurst = 0; alwaysFullBurst < 2; alwaysFullBurst++) {
				if(alwaysFullBurst)
					FPGAMSHR_Enable_AlwaysGetFullBurst();
				else
					FPGAMSHR_Disable_AlwaysGetFullBurst();
...
#else // FPGAMSHR_EXISTS
					cache_divider = CACHE_SIZE_REDUCTION_VALUES + 1;
#endif // FPGAMSHR_EXISTS
#if FPGAMSHR_EXISTS
					// run parameters
					xil_printf("\r\n%s %d %d %d %d %d %d %d %d %d ", benchmarks[benchmark_idx], num_spmv,
					ROB_DEPTH, MSHR_HASH_TABLES, MSHR_PER_HASH_TABLE, SE_BUF_ENTRIES_PER_ROW, SE_BUF_ROWS, CACHE_WAYS,
					cache_divider == CACHE_SIZE_REDUCTION_VALUES ? 0 : CACHE_SIZE >> cache_divider, alwaysFullBurst);
#else // FPGAMSHR_EXISTS
